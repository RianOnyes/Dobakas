        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <div class="flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Permintaan Donasi</h3>
                            <p class="text-gray-600 dark:text-gray-400">Kelola permintaan donasi yang Anda buat untuk menarik perhatian donatur.</p>
                        </div>
                        <a 
                            href="{{ route('organisasi.create-request') }}" 
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    @if($status)
                        dengan status {{ $status === 'active' ? 'aktif' : ($status === 'fulfilled' ? 'terpenuhi' : 'dibatalkan') }}
                    @endif
                </div>
            </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 dark:border-gray-700">
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $request->getUrgencyBadgeClass() }}">
                                ...
                                <div class="flex gap-2">
                                    @if($request->status === 'active')
                                        <form method="POST" action="{{ route('organisasi.request.status', $request) }}" class="flex-1" id="fulfillForm{{ $request->id }}">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form method="POST" action="{{ route('organisasi.request.status', $request) }}" class="flex-1" id="cancelForm{{ $request->id }}">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('organisasi.request.status', $request) }}" class="flex-1" id="reactiveForm{{ $request->id }}">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <form method="POST" action="{{ route('organisasi.request.delete', $request) }}" class="inline" id="deleteForm{{ $request->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button 
                                            type="button" 
                                            class="px-3 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm"
            ...
            @else
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">Belum ada permintaan</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            @if($status)
                                Tidak ada permintaan dengan status {{ $status === 'active' ? 'aktif' : ($status === 'fulfilled' ? 'terpenuhi' : 'dibatalkan') }}.
                            @else
            ...
            @foreach($requests as $request)
                @if($request->status === 'active')
                    ...
                    <x-confirmation-modal 
                        ...
                        confirmText="Ya, Sudah Terpenuhi"
                        cancelText="Batal"

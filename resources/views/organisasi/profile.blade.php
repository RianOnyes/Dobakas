    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Kelola Profil Organisasi</h3>
                    ...
                    <form method="POST" action="{{ route('organisasi.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            >{{ old('organization_address', $organizationDetail->organization_address ?? '') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                rows="4"
                                class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-berkah-teal focus:ring-berkah-teal"
                                placeholder="Ceritakan tentang organisasi Anda, tujuan, dan kegiatan yang dilakukan..."
                            >{{ old('description', $organizationDetail->description ?? '') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Needs List -->
                        <div>
                            <label for="needs_list" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Daftar Kebutuhan
                            </label>
                                    ...
                                    @foreach($organizationDetail->needs_list as $index => $need)
                                        <div class="flex gap-2 need-item">
                                            <input 
                                                ...
                                                class="flex-1 rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-berkah-teal focus:ring-berkah-teal"
                                                placeholder="Contoh: Pakaian Anak, Buku Sekolah, dll"
                                            >
                                            <button type="button" onclick="removeNeed(this)" class="px-3 py-2 bg-red-500 text-white rounded-md hover:bg-red-600">
                                                Hapus
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            <button type="button" id="add-need" class="mt-2 px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">
                                Tambah Kebutuhan
                            </button>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambahkan jenis donasi yang dibutuhkan organisasi Anda</p>
                            @error('needs_list')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Document Upload -->
                        <div>
                            <label for="document_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Dokumen Organisasi
                            </label>
                            @if($organizationDetail && $organizationDetail->document_url)
                                <div class="mb-2 p-3 bg-green-50 dark:bg-green-900 rounded-md">
                                    <p class="text-sm text-green-700 dark:text-green-300">
                                        Dokumen saat ini: 
                                        <a href="{{ asset('storage/' . $organizationDetail->document_url) }}" target="_blank" class="font-medium underline">
                                            Lihat Dokumen
                                        </a>
                                    </p>
                                </div>
                            ...
                            <input 
                                ...
                                class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-berkah-teal focus:ring-berkah-teal"
                            >
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Upload dokumen legitimasi organisasi (PDF, DOC, DOCX, maksimal 2MB)</p>
                            @error('document_url')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end pt-4">
                            <button 
                                type="submit" 
                                class="px-6 py-2 bg-berkah-teal text-white rounded-md hover:bg-berkah-hijau-gelap transition-colors"
                            >
                                Simpan Profil
                            </button>
                        </div>

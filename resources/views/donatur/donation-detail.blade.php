<x-dashboard-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div
                    class="mb-6 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded relative">
                    {{ session('success') }}
                </div>
            @endif
            ...
            @if(session('error'))
                <div
                    class="mb-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded relative">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="mb-6">
                <a href="{{ route('donatur.donasi-saya') }}"
                    class="inline-flex items-center px-4 py-2 bg-gray-100 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-200 transition ease-in-out duration-150">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center space-x-4 text-sm text-gray-600 mb-4">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $donation->status_badge_color }}">
                        ...
                        <div class="flex space-x-2">
                            @if(in_array($donation->status, ['pending', 'available']))
                                <form action="{{ route('donatur.donation.cancel', $donation) }}" method="POST"
                                    class="inline" id="cancelFormDetail">
                                    @csrf
                                    @method('PATCH')
                                    <button type="button"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    @foreach($donation->photos as $photo)
                                        <div class="aspect-w-16 aspect-h-12">
                                            <img src="{{ asset('storage/' . $photo) }}" alt="{{ $donation->title }}"
                    ...
                    @if($donation->description)
                        <div class="bg-slate-100 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">Deskripsi Barang
                                </h3>
                                            ...
                                            <div class="relative flex space-x-3">
                                                ...
                                                <div class="min-w-0 flex-1 pt-1.5">
                                                    ...
                                                    <p class="text-sm text-gray-500">
                                                        {{ $donation->created_at->format('d M Y, H:i') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                        ...
                                        <li>
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex space-x-3">
                                                    ...
                                                    <div class="min-w-0 flex-1 pt-1.5">
                                                        <p class="text-sm text-gray-900 font-medium">
                                                            {{ $donation->status_label }}</p>
                                                        <p class="text-sm text-gray-500">
                                                            {{ $donation->updated_at->format('d M Y, H:i') }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-slate-100 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Detail Donasi</h3>
                            <div class="space-y-4">
                                <div>
                                    <p class="text-sm font-medium text-gray-900 ">Kategori</p>
                                    <p class="text-gray-600">{{ $donation->category }}</p>
                                </div>
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-900 ">Preferensi
                                        Pengambilan</p>
                                    <p class="text-gray-600">{{ $donation->pickup_preference_label }}
                                    </p>
                                </div>
                                ...
                                @if($donation->location)
                                    <div>
                                        ...
                                        <p class="text-gray-600">{{ $donation->location }}</p>
                                    </div>
                                @endif
                                ...
                                @if($donation->updated_at != $donation->created_at)
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 ">Terakhir Diperbarui
                                        </p>
                                        <p class="text-gray-600">
                                            {{ $donation->updated_at->format('d M Y, H:i') }}</p>
                                    </div>
                    ...
                    @if($donation->claimedByOrganization)
                        <div class="bg-slate-100 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">Diklaim Oleh</h3>
                                <div class="space-y-4">
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 ">Nama Organisasi</p>
                                        <p class="text-gray-600">
                                    ...
                                    @if($donation->claimedByOrganization->organizationDetail)
                                        @if($donation->claimedByOrganization->organizationDetail->contact_person)
                                            <div>
                                                ...
                                                <p class="text-gray-600">
                                                    {{ $donation->claimedByOrganization->organizationDetail->contact_person }}</p>
                                            </div>
                                        @endif
                                        ...
                                        @if($donation->claimedByOrganization->organizationDetail->contact_phone)
                                            <div>
                                                ...
                                                <p class="text-gray-600">
                                                    {{ $donation->claimedByOrganization->organizationDetail->contact_phone }}</p>
                                            </div>
                                        @endif
                                    @endif
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-600">
                                            {{ $donation->claimedByOrganization->email }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    @if(in_array($donation->status, ['pending', 'available']))
        <x-confirmation-modal id="cancelModalDetail" title="Batalkan Donasi"
            ...
            confirmText="Ya, Batalkan Donasi" cancelText="Tidak, Tetap Simpan" confirmClass="bg-red-600 hover:bg-red-700" />
    @endif

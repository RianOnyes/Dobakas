...
<body class="h-full bg-gradient-berkah-light">
    <div class="min-h-full relative">
        ...
        <div class="lg:pl-72 relative z-10">
            <!-- Top header -->
            <div
                class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-2 sm:gap-x-4 border-b border-berkah-accent/20 bg-white/90 backdrop-blur-sm px-3 sm:px-4 shadow-sm lg:px-8">
                <!-- Mobile menu button -->
                ...
                <div class="flex flex-1 gap-x-2 sm:gap-x-4 self-stretch lg:gap-x-6">
                    ...
                    <div class="flex items-center min-w-0 flex-1">
                        @isset($header)
                            <h1 class="text-base sm:text-lg lg:text-xl font-semibold leading-6 text-gray-900 truncate">
                                {{ $header }}
                            </h1>
                        @endisset
                    </div>
                    ...
                    <div class="flex items-center gap-x-2 sm:gap-x-4 lg:gap-x-6">
                        ...
                        <div class="relative group">
                            <button type="button" class="-m-1.5 flex items-center p-1.5">
                                ...
                                <div
                                    class="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-gradient-berkah-dark flex items-center justify-center">
                                    <span
                            ...
                            <!-- Dropdown menu -->
                            <div
                                class="opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5">
                                <a href="{{ route('profile.edit') }}"
                                    class="block px-3 py-1 text-sm leading-6 text-gray-900 hover:bg-gray-50">Profil
                                    Kamu</a>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full text-left px-3 py-1 text-sm leading-6 text-gray-900 hover:bg-gray-50">Keluar</button>
                                </form>
                            </div>
                        </div>
                    </div>
    ...
    <div id="mobile-sidebar"
        class="relative z-50 lg:hidden hidden opacity-0 pointer-events-none transition-opacity duration-300">
        <div class="fixed inset-0 bg-gray-900/80" id="mobile-overlay"></div>

        <div class="fixed inset-0 flex">
            <div class="relative mr-16 flex w-full max-w-xs flex-1">
                <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gradient-berkah-dark px-4 pb-4 transform -translate-x-full transition-transform duration-300"
                    id="mobile-sidebar-panel">
                    <!-- Close button -->
                    <div class="flex h-16 shrink-0 items-center justify-between">
                        <div class="flex items-center">
                            <div
                                class="w-8 h-8 bg-white rounded-full flex items-center justify-center text-berkah-primary font-bold text-sm mr-2">
                                BB
                            </div>
                            <div class="text-white">
                                <div class="text-lg font-bold">Berkah BaBe</div>
                                <div class="text-xs text-berkah-accent opacity-75">{{ ucfirst(auth()->user()->role) }}
                                    Dashboard</div>
                            </div>
                        </div>
                        <button type="button" id="close-mobile-sidebar"
                            class="-m-2.5 p-2.5 text-white hover:text-berkah-mint">
                            ...
                            <li>
                                <ul role="list" class="-mx-2 space-y-1">
                                    @if(auth()->user()->isAdmin())
                                        ...
                                        <x-dashboard.nav-item route="admin.statistics" icon="chart"
                                            label="Statistik & Laporan" />
                                        <x-dashboard.nav-item route="admin.donations" icon="donations"
                                            label="Kelola Donasi" />
                                        <x-dashboard.nav-item route="admin.organizations" icon="organizations"
                                    ...
                                    @elseif(auth()->user()->isDonatur())
                                        ...
                                        <x-dashboard.nav-item route="donatur.cari-bantuan" icon="search"
                                            label="Cari Bantuan" />
                                        <x-dashboard.nav-item route="donatur.donasi-saya" icon="donations"
                                            label="Donasi Saya" />
                                    ...
                                    @elseif(auth()->user()->isOrganisasi())
                                        ...
                                        <x-dashboard.nav-item route="organisasi.warehouse-donations" icon="search"
                                            label="Gudang Admin" />
                                        <x-dashboard.nav-item route="organisasi.claimed-donations" icon="donations"
                                            label="Donasi Diklaim" />
                                        <x-dashboard.nav-item route="organisasi.requests" icon="requests"
                                            label="Permintaan Saya" />
                                        <x-dashboard.nav-item route="organisasi.create-request" icon="plus"
                                            label="Buat Permintaan" />
                                        <x-dashboard.nav-item route="organisasi.profile" icon="profile"
                                ...
                                <ul role="list" class="-mx-2 space-y-1">
                                    ...
                                    <li>
                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button type="submit"
                                                class="group flex w-full gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold text-berkah-mint hover:text-white hover:bg-berkah-hijau-gelap transition-colors">

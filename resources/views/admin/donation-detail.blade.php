<x-dashboard-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-slate-100 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h1 class="text-2xl font-bold text-gray-900 mb-4">{{ $donation->title }}</h1>
                    <p class="text-gray-600 dark:text-gray-400 mb-6">{{ $donation->description }}</p>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Informasi Donasi
                            </h3>
                            <div class="space-y-3">
                                <div>
                                    <label
                                ...
                                <div>
                                    ...
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $donation->getStatusBadgeClass() }}">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 mb-4">Informasi Donatur
                            </h3>
                            <div class="space-y-3">
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama</label>
                                    <p class="mt-1 text-sm text-gray-900 ">{{ $donation->user->name }}
                                    </p>
                                </div>
                    ...
                    @if($donation->status === 'pending')
                        <div class="mt-8 border-t pt-6">
                            ...
                            <div class="flex space-x-4">
                                <form method="POST" action="{{ route('admin.donations.status', $donation) }}" class="inline"
                                    id="approveForm">
                                    @csrf
                                    @method('PATCH')
    ...
    <!-- Confirmation Modals -->
    @if($donation->status === 'pending')
        <x-confirmation-modal id="approveModal" title="Setujui Donasi"
            message="Apakah Anda yakin ingin menyetujui donasi '{{ $donation->title }}'? Donasi akan tersedia untuk diklaim oleh organisasi."
            confirmText="Ya, Setujui" cancelText="Batal" confirmClass="bg-green-600 hover:bg-green-700" />

        <x-confirmation-modal id="rejectModal" title="Tolak Donasi"
            message="Apakah Anda yakin ingin menolak donasi '{{ $donation->title }}'? Donasi akan dibatalkan dan tidak dapat dikembalikan."
            confirmText="Ya, Tolak" cancelText="Batal" confirmClass="bg-red-600 hover:bg-red-700" />
    @endif
</x-dashboard-layout>
